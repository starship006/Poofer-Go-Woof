...
/**
 * PremiumMediocrePathing: Perhaps the best you can reasonably hope for when your budget is only a few thousand bytecode.
 *
 * Let's face it: Dijkstra, A*, and Bellman-Ford are shiny but way outside our price range.
 * Bug pathing and BFS are less useful as they don't handle weights, but even those are still too costly.
 *
 * So, what's a poor battlecode programmer to do?
 * Spend the whole month optimising pathing and have no time to develop anything else?
 * Kinda nerdy fun, but perhaps not the optimal route to winning a cash prize.
 *
 * What we need is something reasonably cheap and not awful. A Premium Mediocre solution.
 *
 * Algorithm:
 *   From the target cell, try all possible moves that take you closer to the origin (current location) cell.
 *   This gives some limited wiggle room for going around obstructions, without letting the detours get too wild.
 *   At each move in the path, select the cheapest alternative. Record (cache in fields) the choice for that cell.
 *   So far, so good, but that just tells you the costs, not the direction. A stack of path elements is too costly, so...
 *   For cells immediately adjacent to the origin i.e. the last ones traversed by the target to origin 'recursive' algorithm,
 *   but the first that will be traversed by the bot leaving the origin heading towards the target, encode the direction into
 *   the low bits of the path cost return value. At the end, decode the value back to a direction and go that way.
 *
 ...
 *   For r2=53, max cost around 2800. For r2=20, max cost around 1400.  You may even have enough left for a nice latte.
 */
public class PremiumMediocrePathing {
     ...
     */
    public PremiumMediocrePathing(RobotController robotController) {
     ...
     *
     * @param mapLocation The intended destination.
     * @return The direction to move, or CENTER if no can be determined
     * @throws GameActionException
     */
    public Direction directionTo(MapLocation mapLocation) throws GameActionException {
        if(!rc.onTheMap(mapLocation) || !rc.canSenseLocation(mapLocation) || origin.equals(mapLocation)) {
            return Direction.CENTER;
        }
        // pathing works in terms of offset from the bot's current location
        int dx = mapLocation.x-origin.x;
        int dy = mapLocation.y-origin.y;
    ...
    // generate a latitude/longitude style identifier for a given cell offset.
    private static String name(MapLocation loc) {
        String name = "cell" + Math.abs(loc.y) + (loc.y < 0 ? "S" : "N") + Math.abs(loc.x) + (loc.x < 0 ? "W" : "E");
        return name;
    }

    // code below here is (mostly) auto-generated by PathGen. No, the source for that isn't included.

...
public class MinerStrategy {
    ...
    static void runMiner(RobotController rc) throws GameActionException {
        ...
        for (MapLocation tryLocation:nearbyLocations){
            if (rc.senseLead(tryLocation) > 1) {
                if (maxLeadAmount < rc.senseLead(tryLocation)){
                    maxLeadAmount = rc.senseLead(tryLocation);
            ...
            if (rc.senseGold(tryLocation) * 10 > maxLeadAmount){
                maxLeadAmount = rc.senseGold(tryLocation) * 10;
                targetLocation = tryLocation;
            }
        }
        if (targetLocation!=null){
            //MovementOptions.newPathTowards(rc, targetLocation); //NEW TARGET
